...
/* ============================================================
 * Momentos — animacoes de limiar (25/50/70/100% nas janelas 5h e semanal)
 * Overlay em lv_layer_top: Clawd XL com "humor" por nivel + contador de %
...
static void moment_close_cb(lv_event_t *e)
{
    (void)e;
    moment_close();
}
...
void show_moment(int win, int thr)
{
    ...
    if (thr == 100) {
        ...
        lv_obj_set_pos(g_mo.ring, 4, 4);
        lv_obj_set_size(g_mo.ring, 472, 312);
    ...
    if (thr == 50) {
        ...
        for (int i = 0; i < 2; i++) {
            g_moXPts[i * 2][0] = (lv_point_precise_t){(lv_value_precise_t)(ex[i] - 2), (lv_value_precise_t)(ey - 1)};
            g_moXPts[i * 2][1] =
                (lv_point_precise_t){(lv_value_precise_t)(ex[i] + ew + 2), (lv_value_precise_t)(ey + eh + 1)};
            g_moXPts[i * 2 + 1][0] =
                (lv_point_precise_t){(lv_value_precise_t)(ex[i] + ew + 2), (lv_value_precise_t)(ey - 1)};
            g_moXPts[i * 2 + 1][1] = (lv_point_precise_t){(lv_value_precise_t)(ex[i] - 2), (lv_value_precise_t)(ey + eh + 1)};
            for (int k = 0; k < 2; k++) {
    ...
    lv_obj_t *win_l = mklabel(s, win == 0 ? TRS("JANELA DE 5 HORAS", "5-HOUR WINDOW") : TRS("JANELA SEMANAL", "WEEKLY WINDOW"),
